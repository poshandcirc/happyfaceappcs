...
//    var routineUpdateTask: UIBackgroundTaskIdentifier?
    ...
    static func addEntry(user: PFUser, acneScale: Int, drynessScale: Int, oilinessScale: Int, rednessScale: Int, date: NSDate, isEmpty: Bool) {
        ...
        entryObject.saveInBackgroundWithBlock(ErrorHandling.errorHandlingCallback)
    }
    
    static func deleteEntry(dateCreated: NSDate) {
            ...
            if error == nil {
                if let objects = objects {
                    for object in objects {
                        object.deleteInBackgroundWithBlock(ErrorHandling.errorHandlingCallback)
    ...
    static func addRoutine(user: PFUser, itemName: String, morningUse: Bool, middayUse: Bool, nightUse: Bool, selectiveUse: Bool, additionalNotes: String, currentUse: Bool, isPrescription: Bool, today: NSDate) {
        ...
        routineObject.saveInBackgroundWithBlock(ErrorHandling.errorHandlingCallback)
    }
    
    static func updateRoutine(originalItemName: String, newItemName: String, morningUse: Bool, middayUse: Bool, nightUse: Bool, selectiveUse: Bool, additionalNotes: String, currentUse: Bool, isPrescription: Bool) {
            ...
            if error == nil {
                if let objects = objects {
                    for object in objects {
                        ...
                        object.saveInBackgroundWithBlock(ErrorHandling.errorHandlingCallback)
                        print("successful update")
                    }
                }
            } else {
    ...
    static func archiveRoutine(originalItemName: String, currentUse: Bool) {
        let query = PFQuery(className: ParseRoutineClass)
        query.whereKey(ParseRoutineItemName, equalTo: originalItemName)
        query.findObjectsInBackgroundWithBlock {
            (objects: [PFObject]?, error: NSError?) -> Void in
    ...
    static func deleteRoutine(originalItemName: String) {
        let query = PFQuery(className: ParseRoutineClass)
        query.whereKey(ParseRoutineItemName, equalTo: originalItemName)
        query.findObjectsInBackgroundWithBlock {
            (objects: [PFObject]?, error: NSError?) -> Void in
